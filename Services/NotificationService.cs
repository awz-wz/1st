...
using System.IO;
using PIWebAPIApp.Models; // Для PIConfiguration
...
namespace PIWebAPIApp.Services
{
    public class NotificationService : INotificationService
    {
        ...
        public NotificationService(PIConfiguration config, ITagService tagService)
        {
            _config = config;
        ...
        public async Task<bool> SendEmailNotificationAsync(string toEmail, string tagName, string oldState, string newState, string user, string justification)
        {
            try
            {
                // TODO: Вынести конфигурацию SMTP в appsettings.json
                using var client = new SmtpClient("smtp.gmail.com", 587);
                client.EnableSsl = true;
                client.Credentials = new NetworkCredential("awzwz0401@gmail.com", "nlyu yttq jiix jbmk".Replace(" ", ""));

                var mailMessage = new MailMessage
                {
                    From = new MailAddress("awzwz0401@gmail.com", "PI Tag Manager"),
        ...
        public async Task<byte[]> GeneratePdfReportAsync(ExportPdfRequest request)
        {
            ...
            if (request.Tags != null)
            {
                foreach (var tagInfo in request.Tags)
                {
                    ...
                    {
                        var tagValue = await _tagService.GetTagValueAsync(_config.DataServerWebId, tagInfo.TagName);
